...
<form  action="javascript: fn_modificar_ficha_otro_puesto();" method="post"  id="frm_per">
...
<tr> 
    <td> Cargo a postularse* <font color="#000000">
	  ...
	  {	echo ('<option value ="'.$rowp['codigo'].'">'.substr(ltrim($rowp['cargo']),0,40).'  -  '.substr(ltrim($rowp['direccion']),0,40).' '.$rowp['codigo']."</option>");			
    	}
    ?>
        </select>
...
?></td>
    ...
    ?>
        ...
        <tr>
          <td colspan="4"> <input type="hidden" name="vclave" size="8" maxlength="8"  value="<?=$rs_per['clave']?>" /> </td>
        </tr>
      </tfoot>
...
<script language="javascript" type="text/javascript">
	...
	function fn_modificar_ficha_otro_puesto(){
		var str = $("#frm_per").serialize();
		$.ajax({
			url: 'ajax_modificar_ficha_otro_puesto.php',
			data: str,
			type: 'post',
